...
CREATE TABLE IF NOT EXISTS systemconfig (
    ...
    UNIQUE KEY uk_user_key_platform_active (user_id, `key`, platform, is_active), -- 唯一约束：支持软删除，同一配置只能有一个激活状态
    INDEX idx_user_active (user_id, is_active),         -- 联合索引：加速查询用户的活跃配置
    INDEX idx_key (`key`),                              -- 配置键索引：加速按key查询
    INDEX idx_platform (platform),                      -- 平台索引：加速按平台过滤
    INDEX idx_user_platform_device_active (user_id, platform, device_name, is_active) -- 复合索引：优化设备查询和删除
...
CREATE TABLE IF NOT EXISTS weightrecord (
    ...
    INDEX idx_user_member (user_id, member_id)          -- 联合索引：加速查询某用户的某成员记录
